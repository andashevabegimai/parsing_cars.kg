...
def get_data(html):
    soup = BeautifulSoup(html, 'lxml')
    cars = soup.find('div', 
    class_ ='catalog-list').find_all('a', class_ = 'catalog-list-item')
    # print(cars)
    for car in cars:
        try:
            title = car.find('span',
            class_ = 'catalog-item-caption').text.strip()
        except:
            title = ''
        # print(title)
        try:
            img = car.find('img',
            class_ = 'catalog-item-cover-img').get('src')
        except:
            img = ''
        # print(img)
        try:
...
def main():
    ...
    while i <= number:
        ...
        html = get_html(url_)
        number = int(get_total_pages(html))
        if not BeautifulSoup(html,'lxml').find('div', class_ = 'catalog_list'):
            break
        get_data(html)
        i+=1
    # print(number)
